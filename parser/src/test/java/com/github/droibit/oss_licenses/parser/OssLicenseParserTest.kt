...
class OssLicenseParserTest {
  ...
  fun parse_licensesNotFound() = runTest(testDispatcher) {
    ...
    try {
      ...
      assertThat(e).hasMessageThat()
        .isEqualTo("Third party library license resources generated by OSS Licenses Gradle Plugin does not exist.")
    }
  }
  ...
  fun parseInternal_withIgnoreLibraries() = runTest(testDispatcher) {
    ...
    val result = parser.parseInternal(
      ...
      licensesSource,
      licenseMetadataSource,
      excludedLicenses = setOf("library1"),
    )
    assertThat(result).containsExactly(
  ...
  fun parseInternal_checkCancellation() = runTest(testDispatcher) {
    ...
    job = launch {
      ...
      try {
        parser.parseInternal(coroutineContext, licensesSource, licenseMetadataSource)
        fail("error.")
      } catch (e: Exception) {
